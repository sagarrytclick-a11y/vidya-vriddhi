...
export function EditNewsModal({ isOpen, onClose, news, onUpdate, isUpdating = false }: EditNewsModalProps) {
  ...
  useEffect(() => {
    if (news) {
      ...
      setUploadedImage(news.imageUrl || null)
    }
  }, [news])
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn(adminDialogClass, "max-w-3xl")}>
        <DialogHeader>
          <DialogTitle className="text-white">Edit News</DialogTitle>
        </DialogHeader>
